mkdir -p model
cd model
wget http://models-lab.inf.um.es/files/model-mate/models/graphical-distil-gpt2.tar.gz
tar -xvf graphical-distil-gpt2.tar.gz
rm graphical-distil-gpt2.tar.gz

wget http://models-lab.inf.um.es/files/model-mate/models/codeparrot-classdiagram.tar.gz

wget http://models-lab.inf.um.es/files/model-mate/models/codeparrot-cd-shuffled.tar.gz
