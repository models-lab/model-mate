...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, logger, file_type='train', block_size=1024):
        ...
        cached_file = os.path.join(args.output_dir, file_type + "_langs_%s" % (args.langs) + "_blocksize_%d" % (
        ...
        if os.path.exists(cached_file) and not args.overwrite_cache:
            ...
            for lang in langs:
                ...
                data.extend(['<s> ' + ' '.join(x['function'].split()) + ' </s>' for idx, x in enumerate(dataset) if
                             idx % world_size == local_rank])
